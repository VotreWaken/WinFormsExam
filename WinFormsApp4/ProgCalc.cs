    ...
    public partial class ProgCalc : Form
    {
        ...
        private void DisplayProperConversionSystemAndDisables(object sender, EventArgs e)
        {
            if (inInfinityException != 1)
            {
                ...
                gbConversions.Controls.AddRange(((RadioButton)sender == RbDecButton) ? ctrControlsForDecimalsSystem : ctrControlsForNonDecimalsSystem);
                if (((RadioButton)sender) == RbDecButton)
                {
                    DisableOrEnableNumbers(true, true);
                    DisableOrEnableAlpbets(false);
                    DisableOrEnableTrigonometric(true);

                    Result.Text = ConvertNumberSystem(inCurrentBase, Result.Text, 10);
                    inCurrentBase = 10;
                }
                if (((RadioButton)sender) == RbOctButton)
                {
                    DisableOrEnableAlpbets(false);
                    DisableOrEnableNumbers(true, false);
                    DisableOrEnableTrigonometric(false);

                    Result.Text = ConvertNumberSystem(inCurrentBase, Result.Text, 8);
                    inCurrentBase = 8;
                }
                if (((RadioButton)sender) == RbHexButton)
                {
                    DisableOrEnableNumbers(true, true);
                    DisableOrEnableAlpbets(true);
                    DisableOrEnableTrigonometric(false);

                    Result.Text = ConvertNumberSystem(inCurrentBase, Result.Text, 16);
                    inCurrentBase = 16;
                }
                if (((RadioButton)sender) == RbBinButton)
                {
                    DisableOrEnableAlpbets(false);
                    DisableOrEnableNumbers(false, false);
                    Result.Text = ConvertNumberSystem(inCurrentBase, Result.Text, 2);
                    DisableOrEnableTrigonometric(false);
                    inCurrentBase = 2;
                }
                blNewEntry = true;
        ...
        {
            inConversionForTrigonometry = ((RadioButton)sender == RbDegree) ? 1 : (((RadioButton)sender == RbRadians) ? 2 : 3);
        }
        private void ChangeConversionStatusForNonDecimal(object sender, EventArgs e)
        {
            inConversionForNonDecimal = (((RadioButton)sender) == RbRadians) ? 1 : ((((RadioButton)sender) == rbtDword) ? 2 : ((((RadioButton)sender) == rbtWord) ? 3 : 4));
        }
        ...
        private void MemmoryOperations(object sender, EventArgs e)
        {
            if (inInfinityException != 1)
            {
                ...
                else if ((Button)sender == MemoryClearButton)
                {
                    dblMemory = 0;
